...
function saveReview(bookId, review) {
  if (utilService.loadFromStorage(bookId)) {
    var reviews = utilService.loadFromStorage(bookId);
    reviews.push(review);
    utilService.saveToStorage(bookId, reviews);
  } else {
    reviews = [review];
    utilService.saveToStorage(bookId, reviews);
  }
...
function getBookReviews(bookId) {
  return utilService.loadFromStorage(bookId);
}
